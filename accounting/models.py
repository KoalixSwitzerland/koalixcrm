...
class AccountingPeriod(models.Model):
    ...
    def createPDF(self, raisedbyuser, whatToCreate):
        ...
        organisiationname.appendChild(
            doc.createTextNode(settings.MEDIA_ROOT + userExtension[0].defaultTemplateSet.organisationname))
        ...
        headerPicture.appendChild(
            doc.createTextNode(settings.MEDIA_ROOT + userExtension[0].defaultTemplateSet.logo.path))
        ...
        out.close()
        if (whatToCreate == "balanceSheet"):
            check_output(['/usr/bin/fop', '-c', userExtension[0].defaultTemplateSet.fopConfigurationFile.path, '-xml',
                          settings.PDF_OUTPUT_ROOT + 'balancesheet_' + str(self.id) + '.xml', '-xsl',
                          userExtension[0].defaultTemplateSet.balancesheetXSLFile.xslfile.path, '-pdf',
            ...
            check_output(['/usr/bin/fop', '-c', userExtension[0].defaultTemplateSet.fopConfigurationFile.path, '-xml',
                          ...
                          userExtension[0].defaultTemplateSet.profitLossStatementXSLFile.xslfile.path, '-pdf',
