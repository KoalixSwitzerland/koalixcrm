...
class ProductSelectionLookup(LookupChannel):

    model = Product

    def get_query(self, q, request):
        return Product.objects.filter(Q(title__icontains=q))
    ...
    def format_item_display(self, obj):
        ...
        return u"%s<div><i>%s</i></div>" % (escape(obj.title), escape(obj.price()))
