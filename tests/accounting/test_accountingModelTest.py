...
class AccountingModelTest(TestCase):
    def setUp(self):
        user = User.objects.create(username="Username", password="Userone")
        ...
        bank_loan = Account.objects.create(
            account_number="2000",
        ...
        spendings = Account.objects.create(
            account_number="3000",
            title="Spendings",
            account_type="S",
        ...
        earnings = Account.objects.create(
            account_number="4000",
            title="Earnings",
            account_type="E",
        ...
        AccountingPeriod.objects.create(title="Fiscal year 2026", begin=from_date, end=to_date)
        ...
        Booking.objects.create(
            from_account=investment_capital,
    ...
    def test_sumOfAllBookingsBeforeAccountPeriod(self):
        ...
        self.assertEqual(
            (cash_account.sum_of_all_bookings_before_accounting_period(accounting_period_2026)).__str__(), "4500.00"
        )
        self.assertEqual(
            (spendings_account.sum_of_all_bookings_before_accounting_period(accounting_period_2026)).__str__(),
            "1000.00",
        )
        self.assertEqual(
            (earnings_account.sum_of_all_bookings_before_accounting_period(accounting_period_2026)).__str__(), "500.00"
        )
    ...
    def test_sumOfAllBookingsWithinAccountgPeriod(self):
        ...
        self.assertEqual(
            (cash_account.sum_of_all_bookings_within_accounting_period(accounting_period_2024)).__str__(), "0"
        )
        self.assertEqual(
            (spendings_account.sum_of_all_bookings_within_accounting_period(accounting_period_2024)).__str__(), "0"
        )
        self.assertEqual(
            (earnings_account.sum_of_all_bookings_within_accounting_period(accounting_period_2024)).__str__(), "0"
        )

    def test_overall_liabilities(self):
        accounting_period_2025 = AccountingPeriod.objects.get(title="Fiscal year 2025")
        self.assertEqual((accounting_period_2025.overall_liabilities()).__str__(), "495000.00")

    def test_overall_assets(self):
        accounting_period_2025 = AccountingPeriod.objects.get(title="Fiscal year 2025")
        self.assertEqual((accounting_period_2025.overall_assets()).__str__(), "494500.00")

    def test_overall_earnings(self):
        accounting_period_2025 = AccountingPeriod.objects.get(title="Fiscal year 2025")
        self.assertEqual((accounting_period_2025.overall_earnings()).__str__(), "500.00")

    def test_overall_spendings(self):
        accounting_period_2025 = AccountingPeriod.objects.get(title="Fiscal year 2025")
        self.assertEqual((accounting_period_2025.overall_spendings()).__str__(), "1000.00")
