...
class DeliveryNote(SalesContract):
    ...
    def create_delivery_note(self, calling_model):
        ...
        if type(calling_model) == koalixcrm.crm.documents.contract.Contract:
            ...
            self.template_set = calling_model.default_template_set.delivery_note_template
            self.discount = 0
...
class OptionDeliveryNote(admin.ModelAdmin):
    list_display = (
    'id', 'description', 'contract', 'customer', 'status', 'currency', 'staff',
    ...
    fieldsets = (
        (_('Basics'), {
            'fields': ('contract', 'description', 'customer', 'currency', 'discount', 'tracking_reference', 'status', 'external_reference', 'template_set' )
        }),
    )
    save_as = True
    ...
    def response_add(self, request, obj, post_url_continue=None):
        ...
        return super(OptionDeliveryNote, self).response_add(request, new_obj)
