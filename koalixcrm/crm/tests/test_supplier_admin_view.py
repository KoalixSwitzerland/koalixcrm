...
class TestSupplierAdminView(UITests):
    ...
    def test_supplier_admin(self):
        ...
        try:
            ...
            WebDriverWait(self.sselenium, timeout).until(element_present)
        except TimeoutException:
            print("Timed out waiting for page to load")
        username = self.sselenium.find_element('xpath', '//*[@id="id_username"]')
        password = self.sselenium.find_element('xpath', '//*[@id="id_password"]')
        submit_button = self.sselenium.find_element('xpath', '/html/body/div/article/div/div/form/div/ul/li/input')
        ...
        self.sselenium.get('%s%s' % (self.live_server_url, '/admin/crm/supplier/add'))
        ...
        try:
            ...
            print("Timed out waiting for page to load")
        name = self.sselenium.find_element('xpath', '//*[@id="id_name"]')
        name.send_keys("This is the name of a supplier")
        submit_button = self.sselenium.find_element('xpath', '/html/body/div/article/div/form/div/footer/ul/li[1]/input')
        submit_button.send_keys(Keys.RETURN)
        try:
            element_present = expected_conditions.presence_of_element_located((By.ID,
                                                                               '/html/body/div/article/header/ul/li/a'))
