...
class BookingJSONSerializer(serializers.ModelSerializer):
    ...
    booking_date = serializers.DateTimeField(format="%Y-%m-%dT%H:%M", input_formats=None, allow_null=False)
    ...
    def update(self, booking: Booking, validated_data: dict[str, Any]) -> Booking:
        booking.description = validated_data["description"]
        booking.amount = validated_data["amount"]
        booking.booking_date = validated_data["booking_date"]
        booking.booking_reference = validated_data["booking_reference"]
